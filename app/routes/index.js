...
router.use(session({
  ...
  cookie: {
    maxAge: 30 * 24 * 60 * 60 * 1000  //กำหนดวันหมดอายุ 1000 วัน
  }
}))
...
router.post('/login', (req, res) => {
  ...
  conn.query(sql, params, (err, result) => {
    ...
    if (result.length > 0) {
      ...
      let token = jwt.sign({ id: id, name: name }, secretCode) //สร้างโทเค่น

      req.session.token = token; //เก็บข้อมูลลง session
...
router.post('/addProduct', isLogin, (req, res) => {
  ...
  form.parse(req, (err, fields, file) => {
    ...
    let newPath = 'D://Code/full stack/h12/app/public/images/';
...
router.post('/editProduct/:id', isLogin, (req, res) => {
  let form = new formidable.IncomingForm({
    ...
    minFileSize: 0 // 👈 สำคัญมาก!
  });
  form.parse(req, (err, fields, file) => {
    if (err) throw err;

    let imgFile = file.img && file.img[0];
    let id = req.params.id;

    if (imgFile && imgFile.originalFilename) {
      let ext = path.extname(imgFile.originalFilename); // เช่น .jpg
      let baseName = path.basename(imgFile.originalFilename, ext); // เช่น myfile
      let uniqueName = `${baseName}-${Date.now()}${ext}`; // myfile-1721849999999.jpg

      let filePath = imgFile.filepath;
      let newPath = 'D://Code/full stack/h12/app/public/images/' + uniqueName;

      // ดึงชื่อภาพเก่าจากฐานข้อมูลเพื่อลบ
      let sqlSelect = 'SELECT img FROM tb_product WHERE id = ?';
      conn.query(sqlSelect, [id], (err, result) => {
        if (err) throw err;

        let oldImg = result[0].img;
        let oldImgPath = 'D://Code/full stack/h12/app/public/images/' + oldImg;
      ...
      // ไม่ได้อัปโหลดรูปใหม่ → ไม่เปลี่ยนค่า img
...
router.get('/deleteProduct/:id/:img', isLogin, (req, res) => {
  let newPath = 'D://Code/full stack/h12/app/public/images/' + req.params.img;
...
router.get('/addToCard/:id', (req, res) => {
  ...
  if (req.session.cart == null) { // สินค้าชิ้นแรก
    //first item
    card.push(order);
  } else {  // สินค้นชิ้นที่ 2 ขึ้นไป
    card = req.session.cart;
    let newItem = true;

    for (let i = 0; i < card.length; i++) {
      if (card[i].product_id == req.params.id) {    // เช็คว่าสินค้าที่เพิ่มเข้ามามีอยู่แล้วหรือยัง ถ้ามีอยู่แล้วให้บวกหนึ่ง
        card[i].qty++;
        newItem = false;
      }
    }

    if (newItem) { // กรณีไม่ใช่สินค้าชิ้นแรก และสินค้าชิ้นนั้นยังไม่มีในตระกล้า
      card.push(order);
    }
  }

  req.session.cart = card;
...
router.post('/trackOrder', (req, res) => {
  let sql = 'SELECT * FROM tb_order WHERE phone = ?';
...
router.post('/exportFromStock', isLogin, async (req, res) => {
  ...
  const barcode = req.body['product_barcode'];
  ...
  try {
    const [product, fields] = await conn.query(sql, params);
